...
In essence, we start out with the logarithmic scoring rule, and we add a
few bells and whistles to make it collaborative. We also prefer the
interpretation of a scoring rule as prediction market with an automatic
market maker, which is a better abstraction because it deals more
elegantly with forecasters not being able to input arbitrarily high or
low probabilities.
...
\item
  ...
  and B''
\item
  Comparability: The cummulative relative Brier score doesn't correspond
...
\item
  Comparability: The cummulative relative log score corresponds to bits
...
natural way to do this would be in proportion to their scores. But in
the case of the logarithmic scoring rule, it is not bounded, which would
complicate payoff calculations.\footnote{Readers may think that
...
is a dry if comprehensive introduction. From the perpective of a
programmer seeking to implement a logarithmic market scoring rule,
Cultivate Labs'
\href{https://www.cultivatelabs.com/prediction-markets-guide/how-does-logarithmic-market-scoring-rule-lmsr-work}{How
does the Logarithmic Market Scoring Rule (LMSR) work?} is unusually
short.
...
importance, we incentivize forecaters to predict on more important
...
\href{https://twitter.com/nntaleb}{Nassim Taleb}'s Incerto.
...
have a small incentive to predict sooner rather than later.
...
accumulated while interaccting with their prediction market.
...
Xcts and (100-X)cts rather than \$0 or \$1.
...
market, it could also have been structured such that an invasion of n\%
of Taiwan pays out n cents for each share.
...
W e outlined three improvements to scoring rules as they are currently
